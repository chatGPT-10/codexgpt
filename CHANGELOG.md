...
- Completed Phase 3C production mutation migration: all supported workspace writers can run through the guarded atomic runtime under contract V1, with per-server production composition, deterministic recovery, required terminal audit, authenticated change-set publication, encrypted before-state retention, keyed owner binding, complete undo preflight/reverse transactions, exact V1 projections, and no fallback to direct writes. `legacy` remains the compatibility default. The V2 `query_audit_events` and `undo_change_set` adapters are implemented but public V2 startup remains fail-closed until Phase 3D adds `move_paths` and validates the exact 31-tool surface.
- Restored GitHub CLI configuration and Windows OS-keyring discovery for the default narrowed Bash environment by preserving or deriving only `USERPROFILE`, `APPDATA`, `LOCALAPPDATA`, and `GH_CONFIG_DIR`; arbitrary parent variables, `GH_TOKEN`, and unrelated API credentials remain excluded unless `CODEXPRO_INHERIT_ENV=1` is explicitly enabled.
- Added the Phase 3A internal atomic-transaction kernel with strict external state manifests, installation-key separation, conservative cross-process workspace locks, exact-byte preconditions, same-volume hard-link staging and backups, participant-gated commit, synchronous rollback, persisted crash recovery before atomic-mode workspace reuse, and unconditional blocking of `.codexpro-txn-*` artifacts. `legacy` remains the compatibility default; Phase 3C now supplies the production writer migration described above.
- Added the Phase 3B persistent local audit backend with strict authorization/execution/recovery/administrative events, canonical authenticated JSONL segments, conservative cross-process writer locking, recoverable final-tail quarantine, fail-closed non-tail integrity handling, UTC-date/size rotation, tombstone-before-delete retention, bounded authenticated queries, transaction-participant ordering, and an eighteenth `codexpro_self_test` audit-readiness check. Phase 3C now injects the production audit runtime when atomic or non-legacy Policy configuration requires it; the public V2 query adapter remains gated with the incomplete V2 surface.

...
- Completed P5 long-task/process experience on the existing Windows process kernel. V5 process successes now expose canonical `state` plus an equal `status` compatibility alias over `starting|running|exited|failed|terminated`; delayed starts are truthfully observable, while `start_process` succeeds only after running. Startup revocation and close join and terminate the eventual owned handle. Existing incremental cursor/wait/quota/Job-tree behavior, V3/V4 wire shapes, authority, and exact direct-tool counts `28/31/39/51/52` remain unchanged.
- Added P3 unified code navigation through additive `semantic(operation=navigate)` intents and the V5 `codexgpt(action=navigate_code)` alias. The server now routes definitions, references, implementations, text, filenames, and diagnostics across the owned TypeScript provider, honest lexical fallback, and bounded file discovery; every normalized result reports provider, quality, fallback, and truncation. Existing semantic/search operations, authority, OAuth scopes, and exact V1-V5 direct-tool counts `28/31/39/51/52` remain compatible.
- Added a bounded `WorkspaceContextSnapshot` to standard `open_current_workspace` and `open_workspace` results. It detects supported project manifests, languages, package managers, and build/test/lint/typecheck commands with explicit sources and `confirmed|inferred` confidence; summarizes Git, guidance, Skills, and semantic/process availability; enforces a 12,000-character budget; keeps the bounded text/snapshot projection free of instruction and Skill bodies while retaining structured compatibility fields; and changes `open_workspace` tree discovery to explicit opt-in.
- Unified every registered direct tool and closed-world supertool child behind one immutable tool definition and one authorization/workspace/Policy/approval/execution/audit/render path. Model-facing guidance now states both when to use and when not to use overlapping navigation, mutation, context, and process tools, while preserving V1–V5 tool counts, wire schemas, OAuth scopes, and public result compatibility.
- Clarified the `run_command` versus `start_process` boundary and hardened persistent-process waiting, natural-exit finalization, audit completion, and bounded result retention.
- Added one secret-redacted configuration snapshot across startup, `config explain`, and `doctor --json`; saved-profile validation/migration is now strict and recoverable, and selected `CODEBASE_BRIDGE_HTTP_TOKEN` use receives an executable value-free migration warning.
- Made `CODEBASE_BRIDGE_REPO_ROOT` select the same workspace and saved profile through the supported public entry as `CODEXGPT_ROOT`, while preserving canonical precedence and emitting a path-free migration command only when the compatibility input wins.
- Preserved `CODEXGPT_HOSTNAME` as the selected compatibility source instead of rewriting it to `CODEXGPT_PUBLIC_HOSTNAME`, while keeping the effective hostname and configuration fingerprint identical and adding a value-free migration warning.
- Preserved `NGROK_DOMAIN` provenance and classified it as mode-ambiguous because its historical ngrok-specific name remains effective across all tunnel modes; runtime values and fingerprints are unchanged, and no removal warning is scheduled.

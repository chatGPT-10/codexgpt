...
- Added the internal Phase 3C authenticated change-set store and server-owned mutation commit runtime: HMAC-authenticated manifests, AES-256-GCM before-state blobs, bounded retention, exact transaction/change-set correlation, non-enumerable pending handles, required audit and change-set participants, rollback on missing handles/provider/participant failure, and committed-manifest cleanup proof. Public workspace writers remain on legacy behavior until their dedicated migrations pass; contract V2 remains unavailable until the complete 31-tool Phase 3D gate.

...
- PR CI #119, #122, #124, and #125 progressively exposed terminal-publication, retention self-observation, live identity-lookup ambiguity, and post-child finalization visibility. STEP-371/372 added and observed a bounded finalization lease. Exact-head run `29759707664` then exposed pre-finalization false-stale windows, so STEP-373 replaced it with one exact renewable `worker-lease.json` covering `running` and `finalizing`; result/stop remain authoritative and stop still requires exact live process identity. STEP-374 removed an independent CIM timing defect from the Windows private-stdin control oracle. Exact-head run `29761293686` passed policy and both Ubuntu jobs; Windows Node 24 no longer reported stale, but two tests exceeded fixed 10/30-second observation bounds while the renewable lease still reported live progress. STEP-375 sets test-only JSON observation to one lease duration and terminal observation to lease duration plus 30 seconds; a crashed worker still becomes stale at lease expiry. No production timeout changed. Runner suites pass 33/33 and the Windows host control file passes 10/10 on both managed Node majors; cleanup/process identity passed five consecutive Node 24 runs at 20/20.

...
- STEP-408 completes the Phase 7 Core local implementation candidate through Gate G7-X: zero-setup owned-worker JS/TS, symbol-or-position `semantic`, exact inherited V5=52, quality-labeled fallback, and approval/identity-bound atomic rename. STEP-409 publishes the first candidate and repairs its Linux parent-directory object-generation race; formal closure still requires a successful replacement exact-head run plus real ChatGPT G7-U.

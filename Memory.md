...
- STEP-416 repairs live G7-U blockers in MCP union descriptors, local approval state/contract binding, root resource facts, repository-scale worker memory, workspace summary degradation, and transaction-recovery lock contention. Adversarial review further restricts recovery deferral to an exact verified-live lock owner. Formal closure still requires STEP-416 publication/exact-head CI plus actual user-observable ChatGPT App G7-U.
...
- STEP-416 managed functional `2026-07-24T19-20-52-160Z-phase7-step416-managed-functional-r2-c01bac11` passes 60/60 per major; isolated repository `2026-07-24T19-23-36-411Z-phase7-step416-managed-repository-r2-0f097adb` passes 1/1 per major with the warm threshold unchanged. Build `2026-07-24T19-27-57-319Z-phase7-step416-managed-build-final-efe701ac`, ordinary `2026-07-24T19-29-42-826Z-phase7-step416-ordinary-final-5ee89b91` (1,227/1,229 plus 2 established skips per major), and Smoke `2026-07-24T19-54-59-950Z-phase7-step416-smoke-final-2daab671` (all eight domains per major) pass with zero stderr; policy, package, link, secret, diff, size, and exact-scope gates also pass. Backend U1/U2 and U3 content drift pass; same-content identity remains deterministic-regression evidence, not real App UI acceptance.
...
- Phase 7 Core is not formally closed: the STEP-416 live-G7-U repair candidate still needs publication/exact-head CI, and no current tool surface can perform or observe the required real ChatGPT App **Scan Tools**/cached-App journeys. Backend and regression evidence does not substitute for actual user-observable U1–U6. Large partial dependency graphs remain read-only/quality-labeled and rename fails closed. `npm audit` has zero high/critical findings but retains two moderate transitive findings in the current MCP SDK compatibility line.

## Open items

1. Publish only the reviewed STEP-416 repair and bind its exact head to the complete CI matrix. Then run real ChatGPT Gate G7-U after one **Scan Tools** refresh or App recreation, retain the old 51-tool migration check, and require actual user-observable evidence for U1–U6 before formal closure.

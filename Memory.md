...
- Phase 6 was first published at `efb53118331868686703d28b5ddea55611836b54`; exact-head run `29990618309` matched that SHA but failed on Ubuntu guidance-path compatibility and Windows Node 24 guidance-mutation/runner-retention cases. Repository policy and Windows Node 20 passed.
- STEP-395 implements the bounded CI repair locally. The user authorized one concise Phase 6 CI-fix commit and a second ordinary push; no force push, deployment, release publication, or Phase 7 work is authorized.
...
- Omitted mode defaults to `standard` with readiness `ready`; explicit `CODEXGPT_GUIDANCE_MODE=legacy` remains the one-restart rollback. Omitted `minimal` mode preserves the exact legacy projection because it has no `codex_context`; explicit `standard + minimal` fails closed. STEP-395 closes the three observed first-publication CI gaps locally; formal Phase 6 closure still requires terminal exact-head success for the second published SHA.

## Approved execution boundary

Phase 6 runtime, exact `yaml@2.9.0`, live G6-M/G6-U acceptance, omitted-default activation, full local closure, and one bounded CI-repair commit plus second ordinary push are authorized. The remaining gate is terminal exact-head Ubuntu/Windows Node 20/24 CI for the replacement SHA. Tasks 4B1–4B6 and `workspace` remain deferred. Force push, destructive history changes, deployment, release publication, credential handling, Phase 7, and silent scope expansion remain excluded.
...
- STEP-395 reproduced all three exact-head run `29990618309` failures before repair. Managed Node 20.20.2 and 24.15.0 each passed the 35-test affected suite and build. Detached ordinary run `2026-07-23T09-14-03-949Z-phase6-ci-fix-ordinary-8d8b57a4` passed 1,175/1,177 with zero failures and two established skips per major; detached Smoke run `2026-07-23T09-39-22-115Z-phase6-ci-fix-smoke-ca19c622` passed all eight sections per major with empty stderr. Both runs cleaned temporary state and completed retention without failure. Package dry-run contains 549 files, 18 compiled guidance files, and no tests, `.ai-bridge`, or memory archive.
...
1. Stage only the reviewed STEP-395 CI repair, create one concise English commit, perform the authorized second ordinary push, and require terminal exact-head Repository policy plus Ubuntu/Windows Node 20/24 Build, Regression, Smoke, and Package success before formal closure.
...
- **STEP-395 - Repair Phase 6 exact-head CI gaps:** normalize Windows-style guidance targets on every host, detect same-size same-handle mutation by byte verification, and retry failed observational lease renewals promptly before the fixed lease can expire.

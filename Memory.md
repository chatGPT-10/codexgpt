...
- STEP-408 completes the Phase 7 Core local implementation candidate through Gate G7-X: zero-setup owned-worker JS/TS, symbol-or-position `semantic`, exact inherited V5=52, quality-labeled fallback, and approval/identity-bound atomic rename. STEP-409 repairs Linux parent-directory object reuse; STEP-410 isolates detached-runner lifecycle tests from non-Windows suite-wide concurrency. Formal closure still requires a successful replacement exact-head run plus real ChatGPT G7-U.

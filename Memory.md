...
- PR CI #119, #122, #124, and #125 progressively exposed terminal-publication, retention self-observation, live identity-lookup ambiguity, and post-child finalization visibility. STEP-371/372 added and observed a bounded finalization lease. Exact-head run `29759707664` then passed policy and Ubuntu Node 24 but Ubuntu Node 20 exposed two pre-finalization false-stale windows, proving that finalization-only persistence was incomplete. STEP-373 replaces `finalizing.json` with one exact renewable `worker-lease.json` covering `running` and `finalizing`; result/stop remain authoritative and stop still requires exact live process identity. The same run's Windows Node 24 failure was an independent control-test oracle defect: CIM intermittently returned an empty self command line. STEP-374 uses `[Environment]::CommandLine`, preserving the private-stdin/`-EncodedCommand` proof without WMI timing. Runner suites pass 33/33 and the Windows host control file passes 10/10 on both managed Node majors; cleanup/process identity passed five consecutive Node 24 runs at 20/20.
...
1. Publish STEP-373/374 on the existing Gate X repair pull request, require the complete runtime matrix on the exact new head, then verify the resulting `main` push CI.

...
Phase 3C's internal mutation runtime gives each prepared writer result a non-enumerable server-owned handle. Before any public success, the wrapper installs the visible transaction, persists required terminal audit evidence, commits the audit and authenticated change-set participants, and proves the committed transaction manifest before cleanup. Missing or foreign handles, provider exceptions, audit failure, change-set failure, or mismatched transaction/change-set facts fail closed and trigger rollback. This runtime remains dormant for public writers until each writer is migrated and the writable atomic gate is opened; current public V1 write behavior is still legacy.

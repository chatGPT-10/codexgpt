...
async function runRipgrep(config: CodexGPTConfig, guard: PathGuard, workspace: Workspace, options: SearchOptions): Promise<SearchResult> {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (code && code > 1) {
        reject(new CodexGPTError(stderr.trim() || `ripgrep failed with exit code ${code}`));
        return;
      }
...
async function runNodeSearch(config: CodexGPTConfig, guard: PathGuard, workspace: Workspace, options: SearchOptions): Promise<SearchResult> {
...
export async function searchWorkspace(config: CodexGPTConfig, guard: PathGuard, workspace: Workspace, rawOptions: Partial<SearchOptions>): Promise<SearchResult> {
  ...
  if (await commandExists("rg")) {
    ...
    throw new CodexGPTError("regex search requires ripgrep. Install rg or retry with regex=false.");
  } else {

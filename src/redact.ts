...
export function redactSensitiveText(text: string): string {
  return text
    .replace(CODEXGPT_TOKEN_ASSIGNMENT_PATTERN, (_match, prefix) => `${prefix}[REDACTED_SECRET]`)
    .replace(CODEXGPT_TOKEN_FIELD_PATTERN, (_match, prefix) => `${prefix}[REDACTED_SECRET]`)
...
function isPlaceholderSecret(value: string): boolean {
  ...
  return (
    ...
    normalized.includes("keep-this-codexgpt-token-stable") ||

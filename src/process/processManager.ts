...
import { interruptProcessOutputSchema, listProcessesOutputSchema, processIdV1Schema, readProcessOutputOutputSchema, resizeProcessTerminalOutputSchema, startProcessInputV1Schema, startProcessOutputSchema, terminateProcessOutputSchema, writeProcessInputOutputSchema } from "../tools/schemas/execution.js";
...
export class ProcessManagerV3 implements ToolResourceResolver {
  ...
  constructor(options: { backend: PersistentProcessBackendV3; contextFingerprint: () => string; startResourceResolver?: ToolResourceResolver; audit?: ProcessAuditCoordinatorV3; quota?: OutputQuotaManager; cursorKey?: Buffer; now?: () => number }) {
    this.#backend = options.backend; this.#context = options.contextFingerprint; this.#startResolver = options.startResourceResolver ?? { describe: () => { throw new Error("START_PROCESS_RESOURCE_REQUIRES_EXECUTION_BINDING"); } }; this.#audit = options.audit ?? new ProcessAuditCoordinatorV3(); this.#quota = options.quota ?? new OutputQuotaManager(); this.#now = options.now ?? Date.now; this.#cursor = new OutputCursorCodec({ key: options.cursorKey ?? randomBytes(32), now: this.#now });
  }
  ...
  async start(args: Record<string, unknown>): Promise<Record<string, unknown>> {
    const input = startProcessInputV1Schema.parse(args); if (input.mode !== "full_access") throw new Error("PROCESS_SANDBOX_UNAVAILABLE");
    ...
    expiryTimer.unref?.();
    const record: RecordV3 = { id, generation, context, mode: "full_access", terminal, startedAt, expiresAt: startedAt + lifetimeMs, status: "running", ring: new OutputRing(), handle: null, release: reservation.release, redactors: { stdout: new StreamingRedactor(), stderr: new StreamingRedactor(), terminal: new StreamingRedactor() }, expiryTimer };
    this.#records.set(id, record);
    try {
      const handle = await this.#backend.start({ processId: id, rawArgs: args, command: input.command, cwd: input.cwd, environment: input.environment ?? {}, timeoutMs: input.timeout_ms ?? 30_000, lifetimeMs, terminal,
      ...
      return startProcessOutputSchema.parse({ codexpro_tool: "start_process", codexpro_title: "Start Process", ok: true, data, error: null, meta: createToolMeta(0, [FULL_ACCESS_PROCESS_WARNING_V3]) }) as Record<string, unknown>;
    } catch (error) {
      clearTimeout(expiryTimer);
      try { await record.handle?.terminate(); } catch { }
  ...
  async revokeAll(reason: "policy_revoked" | "evidence_revoked" | "transport_closed" | "lease_revoked" = "policy_revoked"): Promise<void> {
    for (const record of [...this.#records.values()]) {
      if (record.status === "running") await this.#terminate(record, reason);
    }
  }
  ...
  #stateResult(schema: { parse(value: unknown): unknown }, tool: string, title: string, id: string): Record<string, unknown> { const record = this.#owned(id); if (!record) throw new Error("PROCESS_NOT_FOUND"); return schema.parse({ codexpro_tool: tool, codexpro_title: title, ok: true, data: { process_id: id, status: record.status }, error: null, meta: createToolMeta() }) as Record<string, unknown>; }
  async #exit(r: RecordV3, _exitCode: number | null, reason: string): Promise<void> {
  ...
  async #terminate(r: RecordV3, reason: string, bestEffortAudit = false): Promise<void> {
    ...
    if (terminationError && !bestEffortAudit) throw terminationError;
  }

...
function cacheKey(workspace: Workspace, fingerprint: string, config: CodexGPTConfig): string {
...
export async function inspectWorkspace(config: CodexGPTConfig, guard: PathGuard, workspace: Workspace): Promise<WorkspaceAnalysis> {
  if (!config.analysisEnabled) throw new Error("Repository analysis is disabled by CODEXGPT_ANALYSIS=0.");

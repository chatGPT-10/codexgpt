...
- The first stable release is `codexgpt@1.0.0`; the current patch release is `codexgpt@1.0.1`, and package metadata, runtime self-reporting, Git tag, and npm `latest` must remain aligned.
...
The npm badge and package metadata should both report `1.0.1`. Use a source checkout only when you specifically need unreleased `main` behavior.

...
- `docs/superpowers/specs/2026-07-13-inspect-workspace-output-schema-design.md` — locally complete first-principles design for the sixteenth Phase 1 direct `inspect_workspace` slice; publication and exact-head CI remain pending.
- `docs/superpowers/plans/2026-07-13-inspect-workspace-output-schema.md` — Tasks 1–4 completed with focused 10/10, adjacent 79/79, complete 263/263, Build, analysis Smoke, analysis CLI Smoke, HTTP Smoke, Smoke 8/8, native-Windows Stress, package dry-run, and diff checking; Task 5 publication remains pending.
...
Phase 1 implementation started on 2026-07-12. The first fifteen vertical slices are published. Slice 16 direct `inspect_workspace` Tasks 1–4 are locally complete: initial RED 0/10, schema-only GREEN 3/10, focused 10/10, adjacent 79/79, complete 263/263, Build, analysis Smoke, analysis CLI Smoke, standalone HTTP Smoke, complete Smoke 8/8, native-Windows Stress, package dry-run, and diff checking passed. The next approved step is Task 5: run `neat-freak`, repeat all release gates, precisely stage only reviewed Slice 16 files, publish the implementation, verify its exact-head Ubuntu/Windows Node 20/24 CI, then publish and verify a separate durable record. Keep Phase 2, workspace lifecycle expansion, and semantic-provider expansion closed.

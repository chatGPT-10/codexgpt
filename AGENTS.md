...
Phases 1–6 are closed. Phase 4's 4B0 stays blocked/non-production; `workspace` and Tasks 4B1–4B6 stay deferred. Preserve verified Node toolchains; use `Memory.md` for current evidence and next action.

Phase 6 is closed at `31631676fe254962a9a4f14d6e025e3edba82b8d`; exact-head run `30033293444` passed Repository policy plus Ubuntu/Windows Node 20/24 Build, Regression, Smoke, and Package. Its implementation, repair, commit, push, and exact-head authority is exhausted. The 2026-07-23 Phase 7 request authorizes its paired design/TDD plan and project-record reconciliation only; Phase 7 runtime, dependencies, external Provider installation, staging, commit, push, publication, release, and deployment require fresh authorization. Force push, destructive history changes, credential operations, Phase 8+, and unrelated scope expansion remain excluded.

The 2026-07-23 publication authorization permits staging, English commits, ordinary pushes, and exact-head CI repair/retry cycles for the already audited Phase 6 closure records, Phase 7 design/TDD records, and STEP-403 public-help repair. It does not authorize npm registry credentials or release, deployment, Phase 7 runtime/dependencies/Provider installation, force push, destructive history, or unrelated scope expansion. Do not add an evidence-only follow-up commit for a CI run ID.

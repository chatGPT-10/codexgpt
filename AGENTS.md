...
Phase 1 is published and formally closed. Phase 2A implementation commit `e6798b6` is published, but exact-head run `29325407247` failed both Ubuntu regression jobs because a Windows-normalization test mixed simulated `win32` path policy with the host Linux filesystem parser; both Windows jobs passed every gate. A test-only cross-platform fixture repair is in progress. Keep Phase 2B closed until the replacement exact-head Ubuntu/Windows Node 20/24 matrix passes and Phase 2A is formally recorded as closed. Use `Memory.md` for the current head and evidence summary, and the master plan plus `docs/memory/archive/policy-kernel-gate.md` for full records. Destructive operations, system-policy changes, candidate sandbox installation, OAuth 2.1, credential migration, Git remote writes, and Phase 6–9 remain independently gated. Failed gates must be fixed rather than bypassed.

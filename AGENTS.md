...
- Phase 1, Phase 2A, Phase 2B, Phase 3A, and Phase 3B are published. Phase 3C planning commit `02e45fc` passed exact-head CI run `29371007807`; Task 1 contract-version gate is locally implemented and awaiting its scoped publication.
...
Phase 1, Phase 2A, Phase 2B, Phase 3A, and Phase 3B are published. Published Phase 3 history is `75b8d54` (3A), `00cb917` (3B), `70b1060` (fixture discovery repair), `c5b0226` (cross-platform protected-source hashes), `88bd4b9` (atomic installation-state publication), `953e080` (CI-repair closure), and `02e45fc` (Phase 3C plan). Planning run `29371007807` passed Build, Regression, Smoke, and Package on Ubuntu/Windows Node 20/24. Task 1 now defines strict contract version configuration plus immutable exact V1/V2 name sets while keeping public V2 startup fail-closed until Phase 3D supplies `move_paths` and the complete 31-tool registration/schema snapshot. Before the Phase 3C migration gate passes, atomic mode remains read-only (`CODEXPRO_WRITE_MODE=off`) and existing public writers remain reviewed legacy behavior.

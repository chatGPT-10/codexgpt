...
`codexgpt@1.0.0` is published and immutable; corrections require a new semantic version. No post-`1.0.0` implementation or deployment is implicitly authorized. U6 proves replacement Legacy compatibility and OAuth return continuity, not recovery of the deleted original App identity. Credential migration, unrelated Cloudflare/Tunnel/DNS mutation, Phase 7B/7C installs, Task Scheduler/service, sandbox/egress, runtime deployment, destructive history, Phase 9, and unrelated scope remain gated. Do not create an evidence-only commit solely to record a CI run ID.

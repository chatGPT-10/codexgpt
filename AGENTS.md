...
The owner has explicitly authorized the `1.0.0` release cycle: versioning, reviewed staging/commit/push, exact-head CI, merge to `main`, Git tag/GitHub Release, and npm package publication. U6 proves replacement Legacy compatibility and OAuth return continuity, not recovery of the deleted original App identity. Credential migration, unrelated Cloudflare/Tunnel/DNS mutation, Phase 7B/7C installs, Task Scheduler/service, sandbox/egress, runtime deployment, destructive history, Phase 9, and unrelated scope remain gated. Do not create an evidence-only commit solely to record a CI run ID.

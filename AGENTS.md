...
- `docs/memory/archive/phase-3-part-2.md` — active Phase 3 Volume 2 beginning with STEP-278 CI closure.
- Phase 1, Phase 2A, Phase 2B, Phase 3A, and Phase 3B are published. STEP-278 closure commit `953e080` passed exact-head CI run `29370073046` across Ubuntu/Windows Node 20/24; the Phase 3C TDD plan is ready and Phase 3C–3D remain next.
...
Phase 1, Phase 2A, Phase 2B, Phase 3A, and Phase 3B are published. Published Phase 3 history is `75b8d54` (3A), `00cb917` (3B), `70b1060` (fixture discovery repair), `c5b0226` (cross-platform protected-source hashes), `88bd4b9` (atomic installation-state publication), and `953e080` (CI-repair closure). Exact-head run `29370073046` passed Build, Regression, Smoke, and Package on Ubuntu/Windows Node 20/24, closing the Phase 3A/3B CI repair gate. Contract V1 remains the exact 28-tool public surface. Phase 3C must migrate every writer and prepare the persistent runtime, query, undo, and versioned schemas, but public contract V2 must remain fail-closed until Phase 3D supplies `move_paths` and the complete 31-tool snapshot. Before the Phase 3C migration gate passes, atomic mode remains read-only (`CODEXPRO_WRITE_MODE=off`) and existing public writers remain reviewed legacy behavior.

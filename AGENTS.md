...
- Follow the 2026-07-26 Phase 8 [spec](docs/superpowers/specs/2026-07-24-phase-8-oauth-and-public-auth-design.md) and [plan](docs/superpowers/plans/2026-07-24-phase-8-oauth-and-public-auth.md). Local source-checkout evidence is complete through STEP-470: G8-0, Tasks 8A1–8A9, real G8-U Journeys U2–U7, and local G8-X. Full Phase 8 closure still requires separately authorized exact-head CI/publication. Preserve the U6 deleted-App evidence substitution, U7 public-loopback/local-admin boundary, `legacy|oauth` exclusivity, credential-free `authRoutes`, two-App rollback via `auth setup --root`, Windows DPAPI CurrentUser, strict DCR/PKCE and token/session bounds, durable revoke/replay, request-local policy identity, no-deletion recovery, no new execution authority, and exact V1–V5 counts `28/31/39/51/52`.
...
Phase 8 local source-checkout evidence is complete through STEP-470: Core Tasks 8A1–8A9, the dedicated OAuth Tunnel/App, G8-U Journeys U2–U7, and local G8-X. U6 proves replacement Legacy compatibility and OAuth return continuity, not recovery of the deleted original App identity. Continue only after explicit authorization for reviewed staging/commit/push and exact-head CI. Credential migration, unrelated Cloudflare/Tunnel/DNS mutation, Phase 7B/7C installs, Task Scheduler/service, sandbox/egress, release/deployment, registry credentials, destructive history, Phase 9, and unrelated scope remain gated. Do not create an evidence-only commit solely to record a CI run ID.

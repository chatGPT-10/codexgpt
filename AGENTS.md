...
- Phase 1, Phase 2A, Phase 2B, Phase 3A, and Phase 3B are published. Phase 3C Tasks 1-4 and the Task 4 Windows repair are published through `bb5b863`; exact-head run `29378357522` passed. Task 5 transaction-backed multi-file `apply_patch` is the current publication candidate.
...
Phase 1, Phase 2A, Phase 2B, Phase 3A, and Phase 3B are published and closed. Phase 3C Tasks 1-4 and the Task 4 Windows repair are published through `bb5b863`; replacement exact-head run `29378357522` passed all four matrices. Task 5 transaction-backed multi-file `apply_patch` is the current publication candidate and must pass its own exact-head CI before Task 6 begins. Contract V1 remains the exact 28-tool public surface, and contract V2 startup stays fail-closed until Phase 3D supplies `move_paths` and the coherent exact 31-tool snapshot. Writable atomic construction remains fail-closed until the Phase 3C mutation-closure and production-wiring gates pass; writers not yet migrated retain reviewed legacy behavior.

...
- Phase 1, Phase 2A, Phase 2B, and Phase 3A–3C are published. Gate 0 commit `77b1e90` passed run `29402504990`. Phase 3D commits `3000aa6` and `e5d9d27` are published; run `29437987007` passed three complete matrices and exposed the Windows Node 20 source-handle compatibility issue now repaired locally.
...
Phase 1 through Phase 3C are published and closed. Gate 0 commit `77b1e9069798235d674342a2c33a234a4266b564` passed run `29402504990`. Phase 3D commits `3000aa6d88190f31c6b93c35ae59e7889317aae4` and `e5d9d27f37f6fd2b7c1b76c8db38755da32c6ab5` are published. Run `29437987007` passed complete Ubuntu Node 20/24 and Windows Node 24 matrices but exposed Windows Node 20 `EPERM` for an unlinked source name while its original handle remained open. The local repair transfers the verified object handle from source to stage after source-name removal and passes Node 20 move 46/46, all other 91 test files, Node 24 complete 809/810, Stress, static, and package gates. Replacement exact-head CI is required before Phase 3 closes. Exact evidence is in `Memory.md` and STEP-304.

...
function run(args, options = {}) {
  ...
  const result = spawnSync(process.execPath, ['scripts/codexgpt.mjs', ...args], {
    ...
    env: {
      ...process.env,
      NO_COLOR: '1',
      CODEXGPT_HOME: executeHandoffStateHome,
      ...env
    },
    encoding: 'utf8',
    ...spawnOptions
...
const root = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-execute-handoff-'));
const executeHandoffStateHome = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-execute-handoff-home-'));
...
const fakeCodexBin = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-fake-codex-bin-'));
...
fs.writeFileSync(process.env.CODEXGPT_FAKE_CODEX_LOG, JSON.stringify(args, null, 2));
...
const fakeCodexEnv = {
  ...process.env,
  NO_COLOR: '1',
  CODEXGPT_FAKE_CODEX_LOG: fakeCodexLog,
...
const executeStagedRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-execute-staged-untracked-'));
...
const timeoutRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-execute-timeout-'));
...
if (process.platform !== 'win32') {
  const stubbornRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-execute-stubborn-timeout-'));
...
const noisyRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-execute-noisy-'));
...
const watchRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-watch-handoff-'));
...
const watchTimeoutRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-watch-timeout-'));
...
const loopRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-handoff-'));
...
if (!plan.includes('Loop fix plan')) {
  ...
  console.log('CODEXGPT_REVIEW=FAIL');
} else {
  console.log('CODEXGPT_REVIEW=PASS');
}
`, 'utf8');
...
const failedExecutorRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-failed-executor-'));
...
const failedReviewerRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-failed-reviewer-'));
...
const barePassRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-bare-pass-'));
...
const loopStagedRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-staged-change-'));
...
const untrackedRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-untracked-change-'));
...
const boundedUntrackedRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-bounded-untracked-'));
...
const dirtyBaselineRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-dirty-baseline-'));
...
const repeatedSameRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-repeated-same-'));
...
const subdirRepoRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-subdir-repo-'));
...
requireSuccess(spawnSync('git', ['-c', 'user.email=codexgpt@example.invalid', '-c', 'user.name=CodexGPT Smoke', 'commit', '-m', 'init'], { cwd: subdirRepoRoot, encoding: 'utf8' }), 'subdir repo git commit');
...
const subdirUntrackedRepoRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-subdir-untracked-'));
...
const subdirUntrackedRun = run([
  ...
  '--review-command',
  `${quoteArg(process.execPath)} -e "console.log('CODEXGPT_REVIEW=PASS')" --plan-file {{plan_file}}`,
  '--require-clean-git-start',
  '--yes'
]);
...
const subdirOutsideUntrackedRepoRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-subdir-outside-untracked-'));
...
requireSuccess(spawnSync('git', ['-c', 'user.email=codexgpt@example.invalid', '-c', 'user.name=CodexGPT Smoke', 'commit', '-m', 'init'], { cwd: subdirOutsideUntrackedRepoRoot, encoding: 'utf8' }), 'subdir outside untracked repo git commit');
...
const largeDirtyRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-large-dirty-'));
...
requireSuccess(spawnSync('git', ['-c', 'user.email=codexgpt@example.invalid', '-c', 'user.name=CodexGPT Smoke', 'commit', '-m', 'init'], { cwd: largeDirtyRoot, encoding: 'utf8' }), 'large dirty git commit');
...
const unavailableDiffRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-unavailable-diff-'));
...
const largePlanRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-large-plan-'));
...
const stagedRenameRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-staged-rename-'));
...
requireSuccess(spawnSync('git', ['-c', 'user.email=codexgpt@example.invalid', '-c', 'user.name=CodexGPT Smoke', 'commit', '-m', 'init'], { cwd: stagedRenameRoot, encoding: 'utf8' }), 'staged rename git commit');
...
const deletedPlanRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-deleted-followup-'));
...
const implicitDeletedPlanRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-loop-implicit-deleted-plan-'));

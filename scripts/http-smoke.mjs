...
async function expectHttpTokenRequired(name, overrides = {}, options = {}) {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `codexgpt-http-no-token-${name}-`));
  ...
  delete env.CODEXGPT_HTTP_TOKEN;
  delete env.CODEBASE_BRIDGE_HTTP_TOKEN;
  if (!options.keepAllowNoToken) delete env.CODEXGPT_ALLOW_NO_HTTP_TOKEN;
  ...
  if (!result.stderr.includes('CODEXGPT_HTTP_TOKEN is required')) {
    throw new Error(`expected ${name} missing-token failure, got:\n${result.stderr}`);
  }
}

async function listTools(url, token) {
  const client = new Client({ name: 'codexgpt-http-smoke', version: '0.0.0' });
...
await expectHttpTokenRequired('non-loopback', { CODEXGPT_HOST: '0.0.0.0' });
await expectHttpTokenRequired('non-loopback-allow-no-token', { CODEXGPT_HOST: '0.0.0.0', CODEXGPT_ALLOW_NO_HTTP_TOKEN: '1' }, { keepAllowNoToken: true });
await expectHttpTokenRequired('tunnel-mode', { CODEXGPT_TUNNEL_MODE: '1' });
...
function postToolsListWithSession(baseUrl, token, sessionId) {
  return fetch(`${baseUrl}/mcp?codexgpt_token=${encodeURIComponent(token)}`, {
    method: 'POST',
    headers: {
...
const root = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-http-smoke-'));
const profileHome = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-http-profile-home-'));
...
const token = 'codexgpt-http-smoke-token';
...
await fs.writeFile(path.join(profileHome, 'runtime', `${runtimeId}.json`), JSON.stringify({
  ...
  localStatusUrl: `http://127.0.0.1:${port}/?codexgpt_token=${token}&access_token=${runtimeAccessSecret}`,
...
const child = spawn('node', ['dist/http.js'], {
  ...
  env: {
    ...
    CODEXGPT_WIDGET_DOMAIN: 'https://widgets.codexgpt.test',
    CODEXGPT_HOME: profileHome
  },
  stdio: ['ignore', 'pipe', 'pipe']
});
...
try {
  ...
  const queryAuthorized = await fetch(`${baseUrl}/healthz?codexgpt_token=${encodeURIComponent(token)}`);
  if (queryAuthorized.status !== 200) {
    throw new Error(`expected URL-token healthz to return 200, got ${queryAuthorized.status}`);
  }

  const badAdminJson = await fetch(`${baseUrl}/admin/profile?codexgpt_token=${encodeURIComponent(token)}`, {
    method: 'POST',
  ...
  const badMcpJson = await fetch(`${baseUrl}/mcp?codexgpt_token=${encodeURIComponent(token)}`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: '{"jsonrpc":'
  });
  ...
  const hugeMcpJson = await fetch(`${baseUrl}/mcp?codexgpt_token=${encodeURIComponent(token)}`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
  ...
  const home = await fetch(`${baseUrl}/?codexgpt_token=${encodeURIComponent(token)}`);
  ...
  if (!homeText.includes('CodexGPT Local Control') || !homeText.includes('CLI controls') || !homeText.includes('Connect ChatGPT') || !homeText.includes('Runtime guardrails')) {
  ...
  const profileBefore = await fetch(`${baseUrl}/admin/profile?codexgpt_token=${encodeURIComponent(token)}`);
  ...
  const invalidProfile = await fetch(`${baseUrl}/admin/profile?codexgpt_token=${encodeURIComponent(token)}`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({
      tunnel: 'ngrok',
      hostname: 'codexgpt-http-smoke.ngrok-free.app',
  ...
  const profileSave = await fetch(`${baseUrl}/admin/profile?codexgpt_token=${encodeURIComponent(token)}`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({
      tunnel: 'ngrok',
      hostname: 'https://codexgpt-http-smoke.ngrok-free.app/mcp',
      port,
      mode: 'agent',
      bash: 'safe',
      bashTranscript: 'full',
      ...
      toolMode: 'full',
      toolCards: true,
      widgetDomain: 'https://widgets.codexgpt.test',
  ...
  if (
    ...
    savedProfile.hostname !== 'codexgpt-http-smoke.ngrok-free.app' ||
  ...
  const localProfile = await fetch(`${baseUrl}/admin/profile?codexgpt_token=${encodeURIComponent(token)}`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
  ...
  const queryTools = await listTools(`${baseUrl}/mcp?codexgpt_token=${encodeURIComponent(token)}`);
  const queryToolNames = toolNames(queryTools);
  for (const expected of ['server_config', 'codexgpt_self_test', 'codexgpt_inventory', 'open_current_workspace', 'open_workspace', 'workspace_snapshot', 'tree', 'search', 'load_skill', 'git_status', 'git_diff', 'show_changes', 'read_handoff', 'wait_for_handoff', 'codex_context', 'handoff_to_agent', 'handoff_to_codex', 'export_pro_context']) {
  ...
  for (const visualTool of queryToolNames) {
    if (hasWidgetMeta(queryTools, visualTool, toolCardUri) || hasToolCardStatusMeta(queryTools, visualTool)) {
      throw new Error(`${visualTool} exposed widget metadata while CODEXGPT_TOOL_CARDS is off`);
    }
  }
  ...
  const mcpUrl = `${baseUrl}/mcp?codexgpt_token=${encodeURIComponent(token)}`;
  ...
  await expectSessionNotFound(await fetch(`${baseUrl}/mcp?codexgpt_token=${encodeURIComponent(token)}`, {
    headers: {
  ...
  const currentOpened = await withClient(mcpUrl, async (client) => {
    ...
    if (result.structuredContent.codexgpt_tool !== 'open_current_workspace') {
  ...
  await withClient(mcpUrl, async (client) => {
    const inventory = await callTool(client, 'codexgpt_inventory', {
      include_global_skills: false,
      include_mcp_servers: false
    });
    if (inventory.structuredContent.codexgpt_tool !== 'codexgpt_inventory') {
...
const disabledRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-http-disabled-tools-'));
const disabledPort = await getFreePort();
const disabledToken = 'codexgpt-http-disabled-token';
const disabledChild = spawn('node', ['dist/http.js'], {
  cwd: path.resolve('.'),
  env: {
    ...process.env,
    CODEXGPT_ROOT: disabledRoot,
    CODEXGPT_ALLOWED_ROOTS: disabledRoot,
    CODEXGPT_PORT: String(disabledPort),
    CODEXGPT_HTTP_TOKEN: disabledToken,
...
try {
  ...
  const disabledTools = await listTools(`${disabledBase}/mcp?codexgpt_token=${encodeURIComponent(disabledToken)}`);
  ...
  await withClient(`${disabledBase}/mcp?codexgpt_token=${encodeURIComponent(disabledToken)}`, async (client) => {
...
const cliRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-cli-http-smoke-'));
...
const badNoAuth = spawn(process.execPath, [
  ...
  env: {
    ...process.env,
    CODEXGPT_HOME: await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-cli-http-bad-home-'))
  },
  stdio: ['ignore', 'pipe', 'pipe']
});
...
const cliChild = spawn(process.execPath, [
  ...
  env: {
    ...process.env,
    CODEXGPT_HOME: await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-cli-http-home-'))
  },
  stdio: ['ignore', 'pipe', 'pipe']
});
try {
...
const connectionTestChild = spawn(process.execPath, [
  ...
  env: {
    ...process.env,
    CODEXGPT_HOME: await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-connection-test-home-'))
  },
  stdio: ['ignore', 'pipe', 'pipe']
});
...
try {
  ...
  for (const hidden of ['codexgpt', 'codexgpt_self_test', 'write', 'edit', 'apply_patch', 'bash', 'export_pro_context', 'handoff_to_agent', 'handoff_to_codex']) {
  ...
  if (!connectionTestStderr.includes('[CodexGPT] POST /mcp received')) {

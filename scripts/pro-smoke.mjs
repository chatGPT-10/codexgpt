...
const root = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-pro-smoke-'));
const stateHome = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-pro-smoke-home-'));
const proApplyEnv = { ...process.env, CODEXGPT_HOME: stateHome };
const codexgptCli = path.resolve('scripts/codexgpt.mjs');
...
if (!proContext.includes('CodexGPT Context Bundle') || !proContext.includes('demo.txt')) {
  throw new Error('pro context bundle did not include expected content');
}

const selectedRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-pro-selected-'));
...
run([codexgptCli, 'pro-bundle', `--root=${root}`, '--path=demo.txt', '--max-files=4', '--max-total-bytes=80000']);
...
run([codexgptCli, 'pro-apply', `--root=${root}`, '--file=plan.md', '--title', 'Relative Pro Plan'], { cwd: root, env: proApplyEnv });
...
const missingRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-pro-missing-'));
runFail(['scripts/pro-apply.mjs', `--root=${missingRoot}`, '--file=missing-plan.md'], { env: { ...proApplyEnv, CODEXGPT_CALLER_CWD: missingRoot } });
try {

...
const root = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-doctor-smoke-'));
...
for (const args of [['--version'], ['-v'], ['version'], ['start', '--version']]) {
  const version = spawnSync(process.execPath, ['scripts/codexgpt-entry.mjs', ...args], {
    cwd: path.resolve('.'),
    env: { ...process.env, CODEXGPT_HOME: home },
    encoding: 'utf8'
  });
  if (version.status !== 0 || version.stdout.trim() !== packageJson.version) {
    throw new Error(`codexgpt ${args.join(' ')} did not print version ${packageJson.version}\nstdout:\n${version.stdout}\nstderr:\n${version.stderr}`);
  }
}
...
for (const expected of ['CodexGPT doctor', 'Node', 'Build artifacts', 'Local port', 'Ready']) {
  if (!output.includes(expected)) {
    throw new Error(`doctor output missing ${expected}\n${output}`);
  }
}

const invalidRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-doctor-invalid-'));

...
const cli = path.join(projectRoot, 'scripts', 'codexgpt.mjs');
const root = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-analysis-cli-'));
const home = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-analysis-cli-home-'));
...
function run(args) {
  return spawnSync(process.execPath, [cli, ...args], {
    ...
    encoding: 'utf8',
    timeout: 5000,
    env: { ...process.env, NO_COLOR: '1', CI: '1', CODEXGPT_HOME: home }
  });
}
...
try {
  ...
  const nonGitRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'codexgpt-analysis-cli-non-git-'));
  try {

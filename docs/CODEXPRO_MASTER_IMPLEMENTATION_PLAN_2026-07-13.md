...
Phase 1、Policy Kernel、Phase 2A、Phase 2B 与 Phase 3A–3C 已正式关闭。Gate 0 commit `77b1e9069798235d674342a2c33a234a4266b564` 已通过 run `29402504990`。Phase 3D 实现 commit `3000aa6d88190f31c6b93c35ae59e7889317aae4` 与测试可移植性 commit `e5d9d27f37f6fd2b7c1b76c8db38755da32c6ab5` 已发布。Replacement run `29437987007` 的 Ubuntu Node 20/24 与 Windows Node 24 完整 Build、Regression、Smoke、Package 全绿；Windows Node 20 通过 Build 后在 Regression 暴露真实运行时兼容问题：source name 已 unlink 但原 path-opened handle 尚未关闭时，Node 20 的 `lstat(oldPath)` 返回 `EPERM`。当前修复在 stage link 完成身份验证后打开 stage handle，保持原 source handle 直到 source-name removal 完成，随即关闭原句柄并把后续对象证明交给 stage handle，destination 安装后再关闭 stage handle；没有把 `EPERM` 当作不存在，也没有缩短 source removal 前的安全持有期。外部 source replacement 测试移到真实的 post-unlink/pre-manifest 窗口。官方 Windows Node 20.20.2 已通过 move/crash/undo 46/46 与其余 91 个测试文件；Node 24 完整 809/810（0 fail、1 个既有 skip）、Build、受影响 84/84、Stress、静态门 17/17、package 308 entries 与 diff 检查均通过。下一步是发布该 Windows Node 20 兼容修复并要求 replacement exact-head Ubuntu/Windows Node 20/24 Build、Regression、完整 Smoke 与 Package 全绿；成功前不得进入 Phase 4。按用户 2026-07-15 指令，Task 级只运行本地门；按 2026-07-14 扩展授权，采用推荐选项连续推进至 Phase 8。Phase 9、生产部署、真实凭据迁移、破坏性数据/历史操作和规格外扩权仍未授权。

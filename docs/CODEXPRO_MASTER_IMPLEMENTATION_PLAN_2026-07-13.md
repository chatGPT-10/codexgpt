...
Phase 1、Policy Kernel、Phase 2A、Phase 2B 与 Phase 3A–3C 已正式关闭。Gate 0 维护 commit `77b1e9069798235d674342a2c33a234a4266b564` 已通过 exact-head run `29402504990`。当前 Phase 3D 工作树已完成实现和对抗性修复：保留 V1 为默认精确 28 工具契约，并为显式 V2 提供精确 31 工具 catalog、direct/supertool 路由、`move_paths`、认证 Manifest/Change Set V2、syscall-boundary write-ahead recovery、父目录身份/reparse 重验、participant-aware V1/V2 recovery、可恢复 undo、服务级 mutation quiesce/drain、bounded audit query 与 Policy resource composition。native Windows 新鲜证据为 Build、Phase 3D focused 72/72、production runtime 12/12、完整 Node discovery 809/810（0 fail、1 个既有平台 skip）、真实子进程 crash oracle 与多进程锁 30/30、七段新鲜 Smoke、Stress、静态门 17/17、`git diff --check` 和 package dry-run 308 entries。未修改的 protected execute-handoff Smoke 保留此前独立 exit-0 证据；本轮标准重跑会终止控制通道，人工 PowerShell 隔离环境又丢失 fake `codex.cmd` shim，因此不冒充新鲜通过。当前仍只允许整批一次性 stage/commit/push，以及 replacement exact-head Ubuntu/Windows Node 20/24 CI；该 CI 成功前不得进入 Phase 4。按用户 2026-07-15 指令，Task 级只运行本地门，不逐 Task 发布；按 2026-07-14 扩展授权，采用推荐选项连续推进至 Phase 8。Phase 9、生产部署、真实凭据迁移、破坏性数据/历史操作和规格外扩权仍未授权。

...
Phase 1、Policy Kernel、Phase 2A、Phase 2B 与 Phase 3A–3C 已正式关闭。Gate 0 commit `77b1e9069798235d674342a2c33a234a4266b564` 已通过 run `29402504990`。Phase 3D 实现与对抗性修复已作为 commit `3000aa6d88190f31c6b93c35ae59e7889317aae4` 发布，保留 V1 默认精确 28 工具契约并提供显式 V2 精确 31 工具、`move_paths`、认证 Manifest/Change Set V2、syscall-boundary recovery、父目录身份/reparse 重验、participant-aware recovery、可恢复 undo、服务级 mutation drain、bounded audit query 与 Policy resource composition。首个 exact-head run `29436565136` 的四个矩阵均通过 Build，但 Regression 暴露两个纯测试可移植性问题：V1 call fingerprint 纳入了运行时路径/平台 enforcement 证据，Ubuntu replacement fixture 又允许已删除 inode 被复用。当前修复不改生产代码，使用平台中立 wire 归一化和预先分配的不同 replacement 对象；本地 Build、受影响 84/84、完整 809/810（0 fail、1 个既有 skip）通过。下一步是发布该 test-only correction 并要求 replacement exact-head Ubuntu/Windows Node 20/24 Build、Regression、完整 Smoke 与 Package 全绿；成功前不得进入 Phase 4。按用户 2026-07-15 指令，Task 级只运行本地门；按 2026-07-14 扩展授权，采用推荐选项连续推进至 Phase 8。Phase 9、生产部署、真实凭据迁移、破坏性数据/历史操作和规格外扩权仍未授权。

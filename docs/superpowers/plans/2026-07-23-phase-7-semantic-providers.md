...
The Phase 7 Core checkout candidate has completed STEP-416 backend repair and live acceptance. STEP-417 preserves transaction-root production control and the legacy owned-process fallback without cross-root fail-open behavior. Its publication `9f8a593e52b5ed57bd6beb4885617dc85e44f6a3` passed Repository policy, Ubuntu Node 20/24, and Windows Node 24 in exact-head run `30144166480`; Windows Node 20 alone failed when a fresh PowerShell `Add-Type` local-control compilation reached the existing 60-second startup boundary. STEP-418 raises only the production and diagnostic local-control cold-start budget to 120 seconds. Request, process-lifetime, output, ownership, and close bounds are unchanged.
...
STEP-418 TDD first changed the bounded-startup assertions to 120 seconds and failed while production/diagnostic constants remained at 60 seconds. After the two constants changed, current-runtime build and 10/10 local-control/CLI regressions passed. Managed Node 20/24 build and affected tests passed; ordinary run `2026-07-25T04-54-46-291Z-phase7-step418-final-local-gates-e8ed7c24` passed 1,227/1,229 with 2 established skips and 0 failures per major. Its immediately following Node 20 HTTP Smoke encountered a dynamic-port health timeout after the long ordinary sequence, so it is not used as Smoke acceptance. The independent protected run `2026-07-25T05-24-21-474Z-phase7-step418-smoke-isolated-594818f3` passed all eight Smoke domains on both majors with exit 0 and zero stderr. The next legal action is:

1. stage and publish only the reviewed STEP-418 code, regression, plan, archive, and memory scope, excluding the two untracked Phase 8 records and all runtime evidence/state;

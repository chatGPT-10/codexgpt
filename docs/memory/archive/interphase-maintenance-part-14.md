...
## 2026-09-01 — STEP-551: Repair release-candidate Windows test isolation and gate `1.0.5`

**Status:** locally complete. The owner authorized publication of `1.0.5`; external release actions remain pending until the versioned commit has passed exact-head CI.

**Goal:** turn the local Control Plane and P1-P5 handoff into a publishable package without weakening its full Windows regression gate or treating an npm login as publication evidence.

**Files changed:** `scripts/test-execution-profile-manifest.mjs`, `scripts/codexgpt.mjs`, `package.json`, `package-lock.json`, `src/http.ts`, `src/stdio.ts`, `src/server.ts`, `CHANGELOG.md`, `README.md`, `README_ZH.md`, `docs/CODEXGPT_MASTER_IMPLEMENTATION_PLAN_2026-07-13.md`, `Memory.md`, and this archive. No runtime profile, OAuth client/grant, App, Tunnel, DNS, service, or installed Runtime was changed.

**Root cause and implementation:** the complete Windows ordinary gate first rejected five newly added Local Control Plane test files because the fail-closed execution-profile inventory had not classified them. They are now isolated so process/port and temporary-state effects cannot overlap unrelated work. A rerun exposed an existing concurrent-worktree race: `persistent-verification-receipt.test.mjs` was in the two-wide safe shard and failed with `GIT_REPOSITORY_UNSAFE` while exercising an intentionally mutable managed worktree. It now has the required exclusive shard. These are scheduling metadata repairs only: they preserve every test and reduce no production authority, test scope, or tool contract. The first exact-head CI caught the remaining release invariants: package, HTTP, STDIO, and MCP server versions must move together, and diagnostic restart commands must quote their `--root` consistently on Linux and Windows. The three runtime identities now report `1.0.5`, and the restart preview always quotes the root, matching the package/lockfile and cross-platform regression contract; public state remains `1.0.4` until the versioned source reaches the required external gates.

**Verification:** `npm run test:focused -- test/test-execution-profiles.test.mjs test/test-domain-classification.test.mjs test/lifecycle-control-app.test.mjs test/local-admin-settings.test.mjs test/runtime-child-manager.test.mjs test/runtime-ownership.test.mjs test/workspace-control-settings.test.mjs` passed 21/21. The focused receipt/profile set passed 11/11; the same receipt test passed under managed Node 20.20.2 and 24.15.0. Detached run `2026-09-01T20-34-09-042Z-release-1.0.5-ordinary-r3-f83e3d1f` ran the authoritative ordinary domain under both managed majors, exited 0, retained complete output with empty stderr, and cleaned owned temporary state. Detached run `2026-09-01T20-53-09-775Z-release-1.0.5-smoke-608d0305` ran `npm run smoke` under both managed majors, exited 0, retained complete output with empty stderr, and cleaned owned temporary state. An earlier complete-gate run correctly failed first for unclassified Control Plane tests, then once for the receipt concurrency conflict; neither failure published or changed external state.

**Risks, rollback, and next action:** the release candidate is not an npm package, tag, GitHub Release, deployment, or runtime replacement. Revert the version/documentation and profile-manifest changes before publication if a later exact-head CI gate fails; do not alter historic package versions or tags. Next, build and policy/package-check the candidate, commit and push it, require exact-head CI, then merge with a merge commit so the npm source SHA remains an ancestor of `main` before publishing and creating the annotated `v1.0.5` GitHub release.
